...
5. **Locations — In progress.** The current UI creates, lists, and opens Location detail routes through the canonical `/gam-locations` resource, and Event creation consumes Location options. Update and removal actions from the published contract remain pending.
...
| Locations | Current `/gam-locations` create/list/get flow | In progress: add update and removal |
...
3. **In progress — complete GamLocation capabilities.** The feature adapters and Event location selector consume the published canonical `/gam-locations` resource. Add full-replacement editing and reasoned removal, including required authorization and conflict states. Do not add a frontend alias for the legacy route because the backend requirement explicitly rejects that compatibility contract. See [GamLocation records](https://github.com/DudsFerraz/GAM-Bakckend-API/blob/main/docs/requirements/gam-locations/gam-location-records.md).

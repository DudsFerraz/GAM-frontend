...
1. **Member management — Done for current routes.** Search and lifecycle actions include direct registration (`POST /members`), a dedicated detail route (`GET /members/{id}`), activation/deactivation, Coordenação geral and Coordenação do Oratório designation in the list details dialog, and Member presence history (`GET /members/{id}/presences`). The two coordinator transitions use their dedicated permissions and the linked Account's authoritative Role projection without exposing identifiers.
...
3. **Account consultation — Done for the current contract.** The Account view searches Accounts and lists translated access types in cards and a read-only details dialog. Coordinator lifecycle, generic Account-role assignment/removal, assignment lookup, and RBAC catalog inspection remain outside this view.
...
5. **Oratório and Oratorianos — Implemented through additional-form draft and print flows.** The dedicated area lists and creates occurrences, provides specialized planning/team/lifecycle detail, and exposes a mobile-first tracker with independent Member and Oratoriano rosters, an unpaged persistent summary, immediate individual writes, completed-occurrence removal reasons, and guarded atomic quick registration. The Oratoriano section covers minimal registration, ordinary-profile correction, informative attendance summary, paginated attendance history, reasoned deletion, a permission-aware paginated timeline of additional-form metadata, explicit sensitive detail, business-facing draft creation, a responsive five-step editor, complete authoritative draft replacement, reasoned draft deletion with ordered profile navigation and workspace-cache cleanup, and permission-aware print-snapshot creation with immediate PDF download. Print metadata is session-only and cannot be rediscovered after reload; attachments, completion, and revocation remain outside the current UI.
...
| Oratórios and Oratorianos | Occurrence creation/detail/planning/teams/lifecycle; dual-roster tracker and persistent summary; quick registration; ordinary profile/search/history/summary/deletion; Oratório Coordinator designation; metadata-only additional-form history; explicit sensitive detail; draft creation, five-step editing with complete replacement, reasoned draft deletion, and Onda A print-snapshot creation with immediate PDF download | Implemented through the session-only print flow; attachments, completion, and revocation remain pending, and reload rediscovery is not available |
...
5. **In progress with an attachment-contract blocker — complete the specialized Oratório workflow.** The dedicated area consumes the current occurrence, planning, team, tracker, ordinary Oratoriano, reasoned deletion, and Oratório Coordinator designation contracts. The metadata-only additional-form history, explicit sensitive detail, draft creation, and five-step editor are implemented without actor identities or automatic sensitive reads; open detail data is narrowed at runtime before presentation or editing and removed from cache on exit. Complete `PUT` replacement uses explicit mappers, authoritative responses, conflict refetch, and one page-level guard for unsaved changes or temporary print documents. Reasoned draft deletion is implemented with a `ReasonDTO`, authoritative success handling, ordered profile navigation, and exact workspace cleanup. E6/E7 print snapshots are implemented as an Onda A session-only flow with immediate PDF download; attachments, completion, and revocation remain planned, and durable reload rediscovery must wait for attachment discovery and presentable actor references. The frontend must not invent these backend-owned shapes. Missa remains a future specialized workflow and must not be invented from the Event type enum alone.

...
3. **Account access administration — In progress.** The Account view searches Accounts, lists translated access types, and removes an existing type when authorized. The API adapters cover the current role operations, but identifier-based assignment and assignment lookup are not exposed until an accepted business-facing Role selection flow exists; the backend RBAC requirement explicitly keeps role collection/search out of scope for now.
...
| Accounts and Account-role operations | Search, role display, authorized drop; assignment API kept behind an internal-ID boundary | In progress: resolve business-facing Role selection |
...
2. **In progress — resolve business-facing Account-role assignment.** The current backend exposes `POST /accounts/{accountId}/roles` with a Role UUID, while its RBAC requirement excludes role collection/search. Obtain an accepted selector/catalog contract or an explicit product decision before adding assignment UI; never turn the UUID field into user-facing form input. Keep the existing authorized drop flow and its Portuguese reason validation.

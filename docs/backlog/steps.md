...
5. **Oratório and Oratorianos — Implemented through additional-form draft creation and editing.** The dedicated area lists and creates occurrences, provides specialized planning/team/lifecycle detail, and exposes a mobile-first tracker with independent Member and Oratoriano rosters, an unpaged persistent summary, immediate individual writes, completed-occurrence removal reasons, and guarded atomic quick registration. The Oratoriano section covers minimal registration, ordinary-profile correction, informative attendance summary, paginated attendance history, reasoned deletion, a permission-aware paginated timeline of additional-form metadata, explicit sensitive detail, business-facing draft creation, a responsive five-step editor, and complete authoritative draft replacement. Draft deletion, snapshots, attachments, completion, and revocation remain outside the current UI. The reload-safe file lifecycle still depends on attachment/snapshot discovery, typed detail data, and presentable actor references.
...
| Oratórios and Oratorianos | Occurrence creation/detail/planning/teams/lifecycle; dual-roster tracker and persistent summary; quick registration; ordinary profile/search/history/summary/deletion; Oratório Coordinator designation; metadata-only additional-form history; explicit sensitive detail; draft creation and five-step editing with complete replacement | Implemented through draft editing; deletion and file/lifecycle actions remain pending, and reload-safe files are blocked by missing read-contract boundaries |

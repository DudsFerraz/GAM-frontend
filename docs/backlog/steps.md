...
5. **Oratório and Oratorianos — Implemented through additional-form read-only detail.** The dedicated area lists and creates occurrences, provides specialized planning/team/lifecycle detail, and exposes a mobile-first tracker with independent Member and Oratoriano rosters, an unpaged persistent summary, immediate individual writes, completed-occurrence removal reasons, and guarded atomic quick registration. The Oratoriano section covers minimal registration, ordinary-profile correction, informative attendance summary, paginated attendance history, reasoned deletion, a permission-aware paginated timeline of additional-form metadata, and explicit sensitive detail in a dedicated read-only route. Creation, editing, snapshots, attachments, completion, and revocation remain outside the current UI. The reload-safe file lifecycle still depends on attachment/snapshot discovery, typed detail data, and presentable actor references.
...
| Oratórios and Oratorianos | Occurrence creation/detail/planning/teams/lifecycle; dual-roster tracker and persistent summary; quick registration; ordinary profile/search/history/summary/deletion; Oratório Coordinator designation; metadata-only additional-form history; explicit sensitive read-only form detail | Implemented through read-only form detail; editor and files remain pending and reload-safe files are blocked by missing read-contract boundaries |
...
5. **In progress with an additional-form contract blocker — complete the specialized Oratório workflow.** The dedicated area consumes the current occurrence, planning, team, tracker, ordinary Oratoriano, reasoned deletion, and Oratório Coordinator designation contracts. The metadata-only additional-form history and explicit sensitive read-only detail are implemented without actor identities or automatic sensitive reads; the open detail data is narrowed at runtime before presentation and removed from cache on exit. Editor, creation, snapshots, attachments, completion, and revocation remain planned; reload-safe files must wait for attachment and print-snapshot discovery reads, `FormRDTO.data` typed as `FormDraftDTO`, and actor references with presentable names. The frontend must not invent these backend-owned shapes. Missa remains a future specialized workflow and must not be invented from the Event type enum alone.

...
5. **Oratório and Oratorianos — Implemented through additional-form metadata history.** The dedicated area lists and creates occurrences, provides specialized planning/team/lifecycle detail, and exposes a mobile-first tracker with independent Member and Oratoriano rosters, an unpaged persistent summary, immediate individual writes, completed-occurrence removal reasons, and guarded atomic quick registration. The Oratoriano section covers minimal registration, ordinary-profile correction, informative attendance summary, paginated attendance history, reasoned deletion, and a permission-aware paginated timeline of additional-form metadata. Sensitive form detail, creation, editing, snapshots, attachments, completion, and revocation remain outside the current UI. The reload-safe file lifecycle still depends on attachment/snapshot discovery, typed detail data, and presentable actor references.
...
5. **In progress with an additional-form contract blocker — complete the specialized Oratório workflow.** The dedicated area consumes the current occurrence, planning, team, tracker, ordinary Oratoriano, reasoned deletion, and Oratório Coordinator designation contracts. The metadata-only additional-form history is implemented without actor identities or automatic sensitive reads. Detail, editor, creation, snapshots, attachments, completion, and revocation remain planned; reload-safe files must wait for attachment and print-snapshot discovery reads, `FormRDTO.data` typed as `FormDraftDTO`, and actor references with presentable names. The frontend must not invent these backend-owned shapes. Missa remains a future specialized workflow and must not be invented from the Event type enum alone.

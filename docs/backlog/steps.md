...
5. **Oratório and Oratorianos — Implemented except for contract-blocked additional forms.** The dedicated area lists and creates occurrences, provides specialized planning/team/lifecycle detail, and exposes a mobile-first tracker with independent Member and Oratoriano rosters, an unpaged persistent summary, immediate individual writes, completed-occurrence removal reasons, and guarded atomic quick registration. The Oratoriano section covers minimal registration, ordinary-profile correction, informative attendance summary, paginated history, and reasoned deletion. Oratório Coordinator designation is available from Account details. Additional forms remain blocked until the generated contract supplies reload-safe attachment/snapshot discovery, typed detail data, and presentable actor references.
...
5. **Implemented with an additional-form contract blocker — complete the specialized Oratório workflow.** The dedicated area consumes the current occurrence, planning, team, tracker, ordinary Oratoriano, reasoned deletion, and Oratório Coordinator designation contracts. Additional forms must wait for attachment and print-snapshot discovery reads, `FormRDTO.data` typed as `FormDraftDTO`, and actor references with presentable names; the frontend must not invent these backend-owned shapes. Missa remains a future specialized workflow and must not be invented from the Event type enum alone.

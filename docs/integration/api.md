...
The metadata-only additional-form history and explicit sensitive read-only detail are implemented from the checked-in generated contract. Both deliberately omit actor identities because `AccountReferenceRDTO` currently provides only an identifier. The detail action is permission-aware and does not preload content; creation remains absent. Editor, snapshot, and attachment UI are still blocked from a reload-safe lifecycle because the contract does not yet provide enough read information to resume the same workflow after a reload:

...
The required file-discovery routes and response-shape evolution are backend-owned. Until a regenerated artifact supplies these boundaries, the frontend does not create guessed endpoints, handwritten transport DTOs, UUID fields, or file controls that lose attachment and snapshot continuity. Draft creation and editing use only the published `POST` and integral `PUT`; reasoned draft deletion uses the published `DELETE` route, while snapshots, attachments, completion, and revocation remain absent. The accepted target behavior remains documented in the [remaining Oratório fronts specification](../superpowers/specs/2026-07-26-oratorio-remaining-fronts-design.md).

...
The metadata-only additional-form history, explicit sensitive detail, draft creation, complete draft replacement, reasoned draft deletion, and the E6/E7 print slice are implemented from the checked-in generated contract. History and detail deliberately omit actor identities because `AccountReferenceRDTO` currently provides only an identifier. The detail action is permission-aware and does not preload content. `FormRDTO.data` is narrowed with a feature-owned runtime schema before either read-only presentation or editor initialization; incompatible data produces a safe error instead of an empty form. The editor maps validated transport data into control values and maps the complete form back to `FormDraftDTO`, without creating handwritten route or response DTOs. Print metadata and the PDF are handled as a session-only workflow: after `POST /print-snapshots`, the returned metadata seeds an exact in-memory cache key and the PDF is fetched immediately. No list or discovery query is invented, so a reload requires a new snapshot:

...
The metadata-only additional-form history is implemented from the checked-in generated contract. It deliberately omits actor identities because `AccountReferenceRDTO` currently provides only an identifier, and it does not expose a detail action or create action. The remaining sensitive detail, editor, snapshot, and attachment UI is blocked from a reload-safe lifecycle because the contract does not yet provide enough read information to resume the same workflow after a reload:
...
| Typed sensitive detail | `FormRDTO.data` is `{ [key: string]: unknown }` even though `FormDraftDTO` exists | The editor cannot safely populate a typed React Hook Form without creating a competing transport contract. |
| Business-facing actors | `AccountReferenceRDTO` contains only `id` | History cannot identify actors without rendering a UUID. |

The required routes and response-shape evolution are backend-owned. Until a regenerated artifact supplies these boundaries, the frontend does not create guessed endpoints, handwritten transport DTOs, UUID fields, or a partial editor that loses attachment and snapshot continuity. The accepted target behavior remains documented in the [remaining Oratório fronts specification](../superpowers/specs/2026-07-26-oratorio-remaining-fronts-design.md).

...
- Event search, authorized public or restricted creation, an Event details dialog with the complete schedule and location data, a dedicated Event route, authorized Generic Event editing/lifecycle/removal, and Event-presence management use the shared search bar with quick title search, filters for situation/type, and contract-backed ordering. Registration uses business-facing Member search and follows the Event-type attendance window; observation editing and reasoned removal select an existing Presence from the authorized roster. Each action is shown only for its specific effective permission, successful mutations refresh the Event roster and Member history, and Oratório cards link to their specialized detail when allowed.

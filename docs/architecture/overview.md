...
- Member search and activation/deactivation actions, direct registration, a dedicated Member detail route, and paginated presence history. The shared search bar starts with active members; the `Filtrar` action can select active, inactive, or both, while internal person selectors may still include inactive members when authorized.
...
- Account search and consultation of translated access types use the shared search bar: quick search by display name, e-mail through `Filtrar`, and contract-backed ordering. Account details open in a dialog, while the account cards use a responsive two-column layout on larger screens. The dialog has separate dedicated actions for GAM-wide Coordinator and Oratório Coordinator designation. Both resolve the matching Member through the existing e-mail search, choose grant or revoke from the authoritative access projection, require a reason, and never expose generic system-role editing. The Oratório action additionally requires `ORATORIO_COORD_MANAGE` and an active, consistent Member projection.

...
The `components/` root is for components with proven reuse across multiple features, such as `ColorModeToggle`, `SearchClearButton`, and the shared `SearchAndFilter` composition. The shared search component owns only the interaction model; feature-owned configurations and API adapters retain each resource's fields, defaults, presentation maps, and transport validation. Free-text asynchronous searches use the shared 500 ms debounce from [`src/hooks/useDebouncedValue.ts`](../../src/hooks/useDebouncedValue.ts), do not require a `Buscar` button, and expose `Limpar busca` only when a term exists. A component used by only one feature remains inside that feature, even if its props appear generic; deliberate confirmation actions remain explicit when they precede a mutation.

...
The `components/` root is for components with proven reuse across multiple features, such as `ColorModeToggle` and the shared `SearchAndFilter` composition. The shared search component owns only the interaction model; feature-owned configurations and API adapters retain each resource's fields, defaults, presentation maps, and transport validation. A component used by only one feature remains inside that feature, even if its props appear generic.

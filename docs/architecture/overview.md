...
Login and registration use React Hook Form with Zod resolvers and feature-local schemas. `src/components/ui/` contains reusable Radix-based primitives, while `src/components/` contains genuinely cross-feature components. `src/components/AsyncState.tsx` provides the shared loading, error/retry, empty, and forbidden feedback vocabulary for data-driven views. `src/components/PublicNavbar.tsx` is the shared navigation for unauthenticated pages, including links between the public home, login, and registration and the theme toggle. `src/components/OratorioAreaNavigation.tsx` is the cross-feature navigation between occurrences and Oratorianos inside the dedicated product area. The authenticated sidebar links the current Account identity to a read-only profile backed by the already loaded `/accounts/me` context. Application composition belongs to `src/app/`. Feature-specific code is organized under `src/features/account`, `src/features/auth`, and the concrete vertical folders in `src/features/manage/`, with components, hooks, API calls, query keys, schemas, types, and mappings colocated where practical.

The authenticated home (`/home`) is a deliberately minimal composition under `src/features/home/`. Its single GAM panel greets the current Account and presents the translated main access type from the already loaded Account context. On screens at least 768 px wide, the panel also mounts a compact mission carousel that reuses the bundled images and quotations from the login experience; the carousel is absent and inactive below that breakpoint. Navigation remains the responsibility of the authenticated shell, and `/home` does not load Event or solicitation data or introduce a dashboard-specific backend contract.

The management area currently provides these vertical views:

- Member search, activation/deactivation, Coordenação geral and Coordenação do Oratório designation, direct registration, a dedicated Member detail route, and paginated presence history. The list details dialog reads the linked Account's authoritative Roles, applies `COORDINATOR_MANAGE` and `ORATORIO_COORD_MANAGE` independently, requires a reason for each designation transition, and keeps inactive Members unavailable for coordinator management. The shared search bar starts with active members; the `Filtrar` action can select active, inactive, or both, while internal person selectors may still include inactive members when authorized.
- Authenticated membership-solicitation history, self-service submission, detail, and `MEMBER_MANAGE` approval/rejection actions. The history remains accessible, while the self-service submission action is hidden for accounts with `MEMBER_MANAGE` and for applicants whose approved history exposes the linked `memberId`. Because `/accounts/me` does not currently expose the linked Member, a directly registered Member without approved solicitation history cannot be identified through the current Account context; the backend remains authoritative for rejecting an ineligible submission.
- Account search and read-only consultation of translated access types use the shared search bar: quick search by display name, e-mail through `Filtrar`, and contract-backed ordering. Account details open in a dialog, while the account cards use a responsive two-column layout on larger screens. The view does not execute Role assignment or coordinator lifecycle operations; those Member-targeted transitions belong to Member management.
- Event search, authorized public or restricted creation, an Event details dialog with the complete schedule and location data, a dedicated Event route, authorized Generic Event editing/lifecycle/removal, and Event-presence management use the shared search bar with quick title search, filters for situation/type, and contract-backed ordering. Registration uses business-facing Member search and follows Event lifecycle eligibility: `SCHEDULED` and `COMPLETED` accept confirmed attendance without a clock-based window, while administrative closure states remain restricted; observation editing and reasoned removal select an existing Presence from the authorized roster. Each action is shown only for its specific effective permission, successful mutations refresh the Event roster and Member history, and Oratório cards link to their specialized detail when allowed.
- A dedicated Oratório area with occurrence list/creation, specialized detail, fixed programming, full planning replacement, four fixed Member teams, lifecycle/removal actions, and a responsive attendance tracker for Members and Oratorianos. Occurrences reuse the shared Event search bar with a fixed `ORATORIO` type, quick date search using the date shown on each card, situation filters, and supported ordering. Complete dates become an inclusive backend interval in the `America/Sao_Paulo` civil day; partial terms such as a month or year are matched locally after all compatible Event-search pages are loaded and are then paginated in groups of 12. The tracker loads both paginated rosters plus the unpaged present summary in parallel, persists each checkbox independently, reconciles authoritative queries after writes, requires a reason for completed-occurrence removals, and offers an atomic quick-registration flow only after a similar-name check. Oratorianos have a separate shared search bar with quick name search and the contract-supported annual-attendance ordering, minimal registration, ordinary-profile correction, informative attendance summary, paginated attendance history, permission-aware deletion with preserved attendance and draft-artifact cleanup, and a separately paginated metadata-only additional-form history under `ORATORIANO_FORM_GET`. The history is isolated from the rest of the profile, preserves the previous page while updating, omits actor identifiers, and never preloads sensitive detail. An explicit history action opens dedicated sensitive detail; the feature validates the open `data` boundary with Zod, disables background refetch, and removes the exact sensitive query on exit. With `ORATORIANO_FORM_MANAGE`, the profile also creates a draft after a business-facing origin choice, seeds its authoritative response, and opens a five-step editor. A single React Hook Form retains every step, explicit mappers replace the complete `FormDraftDTO`, successful writes reset dirty state from the authoritative response, conflicts refetch once, and one page-level guard covers local changes and in-memory print documents. The editor stepper evaluates the completion schema continuously and marks visited sections as completed, invalid, or pending; future sections remain visually muted until visited. Forward navigation checks required fields, backward navigation remains free, and draft saving remains incremental. Immutable and unknown statuses stay read-only. Editable drafts also support reasoned deletion with authoritative success, ordered navigation to the Oratoriano profile, and exact workspace-cache cleanup. Authorized users with `ORATORIANO_FORM_PDF_GENERATE` can create an immutable print snapshot and download its PDF immediately; metadata remains memory-only and is not rediscovered after reload. Attachments, completion, and revocation remain outside the current UI, as described in [API integration](../integration/api.md#additional-form-contract-gate).
...
Forms use feature-local React Hook Form and Zod schemas. Required fields opt into the shared `FormItem required` context, which adds the visual `*`, the Portuguese assistive description, and `required`/`aria-required` semantics through `FormLabel`, `FormLegend`, and `FormControl`; conditional fields compute that prop from their current business state. Forms with explicit schema validation use `noValidate` so Zod/RHF messages remain the user-facing validation source. Search and filter controls remain unmarked when their empty state is valid. Paginated pages share the domain-neutral pagination component and deliberately render loading, empty, error, forbidden, and retry states. Contract enums, roles, permissions, catalog text, and errors cross a Portuguese presentation boundary before rendering; granular authorization data and technical identifiers are not profile or management content. See [user-facing language and presentation](../guides/user-facing-language.md).
...
│       ├── oratorianos/ # Ordinary Oratoriano profiles and attendance history
│       ├── oratorios/   # Occurrences, planning, teams, lifecycle, and tracker
...
The Oratório area has three narrow cross-feature boundaries. Its occurrence list reuses the Event search hook and Event query prefix because the backend exposes no specialized Oratório search route, while Oratório mutations invalidate that shared list. The tracker quick-registration mutation seeds and invalidates the Oratoriano detail/list query boundaries. The additional-form detail consumes only the public Oratoriano query and name presentation to identify the person without duplicating ordinary-profile transport. The shared area navigation lives in `src/components/`; transport calls, attendance rules, name equivalence, and presentation mappings remain in their owning feature. The additional-form page composes the workspace guard/cache cleanup hook, the read-only detail view, and the editor stepper; the large editor and quick-registration flows keep their form, results, and confirmation responsibilities in separate feature components.
...
The `components/` root is for components with proven reuse across multiple features, such as `ColorModeToggle`, `SearchClearButton`, and the shared `SearchAndFilter` composition. The shared search component owns only the interaction model; feature-owned configurations and API adapters retain each resource's fields, defaults, presentation maps, and transport validation. A field configuration may declare a value validator when the generated contract has non-obvious length or format rules. The shared component displays its Portuguese message accessibly and keeps the invalid filter local instead of sending a request that would replace the results with a loading error. Free-text asynchronous searches use the shared 500 ms debounce from [`src/hooks/useDebouncedValue.ts`](../../src/hooks/useDebouncedValue.ts), do not require a `Buscar` button, and expose `Limpar busca` only when a term exists. A component used by only one feature remains inside that feature, even if its props appear generic; deliberate confirmation actions remain explicit when they precede a mutation.

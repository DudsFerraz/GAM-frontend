...
The Oratório area has two narrow cross-feature boundaries. Its occurrence list reuses the Event search hook and Event query prefix because the backend exposes no specialized Oratório search route, while Oratório mutations invalidate that shared list. The tracker quick-registration mutation seeds and invalidates the Oratoriano detail/list query boundaries. The shared area navigation lives in `src/components/`; transport calls, attendance rules, name equivalence, and presentation mappings remain in their owning feature.

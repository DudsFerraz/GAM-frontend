...
| Member list/detail/dialogs (`/manage/members/*`) | Situation and embedded Event types translated; technical identifiers removed; Account and Presence registration targets are chosen through business-facing search; activation and deactivation require a business-facing reason. |
...
| Account consultation (`/manage/accounts`) | Account details and translated access types are shown in a dialog; GAM-wide and Oratório Coordinator actions are independently permission-gated, chosen from the current access projection, and require a business-facing reason. Inactive, absent, or inconsistent Member relationships receive safe non-action states. The view has no generic role-editing controls, RBAC inspector, permission catalog, UUID entry, or assignment-identifier lookup. |

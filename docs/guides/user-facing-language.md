...
| Event list/detail/creation/management (`/manage/events/*`) | Event details open in a dialog with translated situation, type, and audience; the dedicated route exposes status-aware Generic Event editing, lifecycle, and removal only with the effective management capability; required reasons and contract conflicts use business-facing Portuguese; permission codes and backend labels are hidden; map links use coordinates or the localized address. |

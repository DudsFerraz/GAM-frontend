...
The frontend will be a versioned static artifact. Fingerprinted assets may be cached immutably, while `index.html` must be revalidated; API routes must never receive SPA fallback. The frontend-owned artifact, configuration boundary, and release handoff are documented under [Frontend Deployment](../deploy/README.md). Canonical proxy selection, VPS provider, domain, production composition, deployment execution, backups, and operations remain backend-owned shared decisions. See [Initial Production Topology](https://github.com/DudsFerraz/GAM-backend-API/blob/main/docs/diagrams/initial-production-topology.md).

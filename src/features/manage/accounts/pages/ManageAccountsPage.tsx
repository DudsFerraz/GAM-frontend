...
export function ManageAccountsPage() {
  ...
  const canInspectRbac = permissions.includes('ROLE_GET') && permissions.includes('PERMISSION_GET')
  ...
  return (
    <div className="space-y-6 py-2 sm:py-4">
      <div><p className="text-sm font-medium text-primary">Administração</p><h1 className="font-heading text-2xl font-bold tracking-tight sm:text-3xl">Contas e papéis</h1><p className="mt-1 text-sm text-muted-foreground">Localize contas, gerencie vínculos de papéis e consulte o contexto RBAC.</p></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]">
        <section className="space-y-4" aria-label="Resultados de contas">
          ...
          {query.isError && (isForbiddenError(query.error) ? <ForbiddenState description="A busca de contas exige a permissão ACCOUNT_SEARCH." /> : <ErrorState onRetry={() => void query.refetch()} />)}
          ...
          {items.map((item, index) => (
            <Card className={selectedAccount?.id === item.id ? 'gap-3 border-primary py-4' : 'gap-3 py-4'} key={item.id ?? index}>
              ...
              <CardContent className="flex flex-wrap gap-1 px-5">{item.roles.map((role, roleIndex) => <Badge key={role.id ?? roleIndex} variant="outline">{role.name}</Badge>)}</CardContent>
        ...
        <aside aria-label="Administração da conta selecionada">
          {selectedAccount ? <AccountAdministrationPanel account={selectedAccount} canInspectRbac={canInspectRbac} canManageRoles={canManageRoles} /> : <EmptyState className="sticky top-0" title="Selecione uma conta." description="Os papéis e vínculos da conta serão exibidos aqui." />}
        </aside>
      </div>
    </div>
  )

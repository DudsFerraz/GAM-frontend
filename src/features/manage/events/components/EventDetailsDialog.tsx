      ...
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-2xl">
        ...
        {query.isError &&
          (isForbiddenError(query.error) ? (
            <ForbiddenState
              className="min-h-48"
              description="Sua conta não tem acesso aos detalhes deste evento."
            />
          ) : (
            <ErrorState
              className="min-h-48"
            ...
            <dl className="grid gap-4 text-sm sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                {event.gamLocation && (
                  <p className="mt-1 text-muted-foreground">
                    {[
                      event.gamLocation.street,
                      event.gamLocation.postalCode,
                      event.gamLocation.city,
                      event.gamLocation.state,
                      formatCountryName(event.gamLocation.countryCode),
                    ]
                      .filter(Boolean)
              ...
              <div className="sm:col-span-2">
                ...
                <dd className="mt-1 whitespace-pre-wrap font-medium">
                  {event.description || "Não informada"}
                </dd>
              </div>
              ...
              {event.gamLocation?.latitude != null &&
                event.gamLocation?.longitude != null && (
                  <div className="sm:col-span-2">
                    <dt className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <dd className="mt-1 font-medium">
                      {event.gamLocation.latitude},{" "}
                      {event.gamLocation.longitude}
                    </dd>
                  </div>
                )}
            </dl>
        ...
        <DialogFooter>
          ...
          {event?.id && (
            <Button asChild type="button">
              <Link params={{ eventId: event.id }} to="/manage/events/$eventId">
                Abrir evento
              </Link>
            </Button>
          )}

...
export type EventFilters = { title: string; status: EventStatus | 'ALL'; type: EventType | 'ALL' }

export async function searchEvents(filters: EventFilters, page: number): Promise<EventPage> {
  const apiFilters: components['schemas']['SpecificationFilterDTO'][] = []
  if (filters.title.trim()) apiFilters.push({ field: 'title', value: filters.title.trim(), comparationMethod: 'LIKE' })
  if (filters.status !== 'ALL') apiFilters.push({ field: 'status', value: filters.status, comparationMethod: 'EQUALS' })
  if (filters.type !== 'ALL') apiFilters.push({ field: 'type', value: filters.type, comparationMethod: 'EQUALS' })
  const { data } = await api.post<EventPage>('/events/search', { filters: apiFilters }, {
...
export async function getEventPresences(eventId: string, page: number): Promise<PresencePage> {
